...
void PLReporter::updateWL()
{
  ...
  // update
  total_hpwl_ = total_hpwl;
  max_hpwl_ = max_hpwl;
  total_stwl_ = total_stwl;
  max_stwl_ = max_stwl;
  long_net_cnt_ = long_net_cnt;
}
...
void PLReporter::printTimingInfo()
{
  for (std::string clock_name : _external_api->obtainClockNameList()) {
    ...
    LOG_INFO << clock_name << " early_wns: " << std::to_string(early_wns) << " early_tns: " << std::to_string(early_tns)
             << " late_wns: " << std::to_string(late_wns) << " late_tns: " << std::to_string(late_tns);
  }
}
...
void PLReporter::reportEDAEvaluation()
{
  ...
  report_stream << design_name << "," << inst_cnt << "," << fix_inst_cnt << "," << net_cnt << "," << pin_cnt << "," << core_area << ","
                << place_density[0] << "," << pin_density[0] << "," << bin_number << "," << bin_size << "," << overflow_number << ","
                << overflow << "," << HPWL[0] << "," << STWL[0] << "," << GRWL[0] << "," << congestion[0] << "," << tns[0] << "," << wns[0]
                << "," << suggest_freq[0] << "," << total_movement << "," << max_movement << "," << pin_density[1] << "," << HPWL[1] << ","
                << STWL[1] << "," << GRWL[1] << "," << congestion[1] << "," << tns[1] << "," << wns[1] << "," << suggest_freq[1] << ","
                << inst_cnt << "," << fix_inst_cnt << "," << net_cnt << "," << pin_cnt << "," << core_area << "," << place_density[2] << ","
                << pin_density[2] << "," << HPWL[2] << "," << STWL[2] << "," << GRWL[2] << "," << congestion[2] << "," << tns[2] << ","
                << wns[2] << "," << suggest_freq[2] << "," << sta_update_runtime << std::endl;
...
void PLReporter::reportTDPEvaluation()
{
  ...
  report_stream << design_name << "," << inst_cnt << "," << fix_inst_cnt << "," << preset_period << "," << target_density << "," << bin_num
                << "," << gp_overflow << "," << HPWL[0] << "," << STWL[0] << "," << pin_density[0] << "," << congestion[0] << "," << tns[0]
                << "," << wns[0] << "," << suggest_freq[0] << "," << lg_total_movement << "," << lg_max_movement << "," << HPWL[1] << ","
                << STWL[1] << "," << pin_density[1] << "," << congestion[1] << "," << tns[1] << "," << wns[1] << "," << suggest_freq[1]
                << "," << HPWL[2] << "," << STWL[2] << "," << pin_density[2] << "," << congestion[2] << "," << tns[2] << "," << wns[2]
                << "," << suggest_freq[2] << std::endl;
